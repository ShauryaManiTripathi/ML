...
from dictionariesandwords import professional_terms,useless_words
...
def calculate_similarityINTERSECTION(job_desc, resumes):
...
def calculate_similarity(job_desc, resumes,unclean_resumes,model, weights=(0.3, 0.5, 0.2)):
    
    # Calculate individual scores
    cosine_scores = calculate_similarityCOSINE(job_desc, resumes, model)
    overlap_scores = calculate_similarityINTERSECTION(job_desc, resumes)
    quality_scores = calculate_similarityQUALITY(unclean_resumes)
    
    combined_scores = []
    
    for scores in zip(cosine_scores, overlap_scores, quality_scores):
...
def upload_files():
    if request.method == 'POST':
        ...
        print(job_desc_text)
        # Process resumes
        unclean_resume_texts=[extract_text_from_pdf(r) for r in resume_paths]
        ...
        similarities = calculate_similarity(job_desc_text, resume_texts,unclean_resume_texts ,model)
        print("====================================================================================\n")
        print(similarities)
